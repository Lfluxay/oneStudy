...
class HandleSign:
    ...
    def encrypt(cls, msg):
        """
        非对称加密
        :param msg:待加密的字符串或字符
        :return:密文
        """
        msg = msg.encode('utf-8')
        pub_key = cls.server_pub.encode('utf-8')
        # 创建publickey对象
        pub_key_obj = rsa.PublicKey.load_pkcs1_openssl_pem(pub_key)
        # 生成加密文本
        cryto_msg = rsa.encrypt(msg, pub_key_obj)
        # 将密文本转化为base64编码
        cipher_base64 = base64.b64encode(cryto_msg)
        # 将字节类型的base64编码转换为字符串类型
...
if __name__ == "__main__":
    ...
    crypt_info = HandleSign.generate_sign(token)
    print(crypt_info)
    s = "1234567"